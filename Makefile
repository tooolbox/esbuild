...
platform-windows:
	...
	GOOS=windows GOARCH=amd64 go build -o npm/esbuild-windows-64/esbuild.exe

platform-darwin:
	mkdir -p npm/esbuild-darwin-64/bin
	cd npm/esbuild-darwin-64 && npm version "$(ESBUILD_VERSION)" --allow-same-version
	GOOS=darwin GOARCH=amd64 go build -o npm/esbuild-darwin-64/bin/esbuild

platform-linux:
	mkdir -p npm/esbuild-linux-64/bin
	cd npm/esbuild-linux-64 && npm version "$(ESBUILD_VERSION)" --allow-same-version
	GOOS=linux GOARCH=amd64 go build -o npm/esbuild-linux-64/bin/esbuild

platform-wasm:
	GOOS=js GOARCH=wasm go build -o npm/esbuild-wasm/esbuild.wasm
